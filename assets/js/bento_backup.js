    ...
    function initializeDashboard() {
      ...
      if (refreshBtn) {
        refreshBtn.addEventListener('click', () => {
          ...
          // Reload data (you can add actual data refresh logic here)
          location.reload();
        });
      }
      ...
      // Timeline items click handler for popup
      const timelineItems = document.querySelectorAll('.timeline-item');
      timelineItems.forEach(item => {
        item.addEventListener('click', (e) => {
          e.preventDefault();
          
          const title = item.querySelector('.timeline-title')?.textContent || '詳細情報';
          const body = item.querySelector('.timeline-body')?.innerHTML || '';
          
          if (popupTitle && popupBody && popupModal) {
  ...
  window.setTimeout(setHeaderHeightVar, 250);
      ...
      // Update Hero Card with Count Up Animation
      const 成分CountEl = document.getElementById("成分-count");
      const 累計DaysEl = document.getElementById("累計-days");
      const 累計YearsEl = document.getElementById("累計-years");
      
      if (成分CountEl) {
        // Animate count from 0 to 9
        animateCountUp(成分CountEl, 9, 1500);
      }
      
      if (累計DaysEl && 累計YearsEl) {
        // Animate total days
        animateCountUp(累計DaysEl, careerSpan.totalDays, 2000);
        
        // Update years/months/days after animation
        setTimeout(() => {
          if (累計YearsEl) {
            累計YearsEl.textContent = `（${careerSpan.years}年${careerSpan.months}ヶ月${careerSpan.days}日）`;
          }
        }, 2000);
      }
      ...
      const baseColors = [
        cssVar("--accent-primary", "#00d4ff"),     // Food
        cssVar("--cat-system", "#a855f7"),             // System  
        cssVar("--cat-store", "#f97316"),              // Store
        cssVar("--cat-retail", "#22c55e"),             // Retail
        cssVar("--cat-fin", "#e11d48"),                // Finance
        cssVar("--cat-wood", "#facc15"),               // Wood
        cssVar("--accent-primary", "#00d4ff"),      // Light
        cssVar("--cat-auto", "#8b5cf6"),               // Auto
        cssVar("--cat-azen", "#f97373")                // A-Zen
      ];
const colorByLabel = {};
        ...
        const barChart = new Chart(barCtx.getContext("2d"), {
          ...
          data: {
            ...
            datasets: [{
              label: "日数",
              data: barDays,
              backgroundColor: barColors.map(color => color + '99'), // Add transparency
          ...
          options: {
            ...
            plugins: {
              tooltip: {
                ...
                borderColor: getComputedStyle(document.documentElement).getPropertyValue('--border-primary'),
                borderWidth: 1
              },
              legend: {
                position: "bottom",
